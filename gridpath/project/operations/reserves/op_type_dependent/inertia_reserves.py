...
RESERVE_PROVISION_RAMP_RATE_LIMIT_COLUMN_NAME_IN_INPUT_FILE = (
    "inertia_reserves_ramp_rate_limit"
)
# Model components
RESERVE_PROVISION_VARIABLE_NAME = "Provide_Inertia_Reserves_MW"
RESERVE_PROVISION_RAMP_RATE_LIMIT_CONSTRAINT_NAME = (
    "Inertia_Reserves_Provision_Ramp_Rate_Limit_Constraint"
)
RESERVE_PROVISION_RAMP_RATE_LIMIT_PARAM_NAME = "inertia_reserves_ramp_rate_limit"
...
def get_inputs_from_database(scenario_id, subscenarios, subproblem, stage, conn):
    ...
    prj_ramp_rates = c.execute(
        """SELECT project, inertia_reserves_ramp_rate
...
def validate_inputs(scenario_id, subscenarios, subproblem, stage, conn):
    ...
    write_validation_to_database(
        ...
        severity="Mid",
        errors=validate_values(df, ["inertia_reserves_ramp_rate"], min=0, max=1),
    )
...
def write_model_inputs(
    ...
    with open(
        ...
        header = next(reader)
        header.append("inertia_reserves_ramp_rate")
